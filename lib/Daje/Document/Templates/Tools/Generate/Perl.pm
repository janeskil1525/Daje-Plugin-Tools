...
sub load_[% project_name %]_[% table.table_name %]  ($self) {
    $self->app->log->debug('Daje::Controller::Super::[%- class_name -%]::load_[% project_name %]_[% table.table_name %] ');
    $self->render_later;
    ...
    $self->tools_objects_index->load_tools_object_fkey_p($tools_objects_fkey)->then(sub($result) {
        ...
        $self->app->log->error('Daje::Controller::[%- class_name -%]::load_[% project_name %]_[% table.table_name %] ' . $err);
